...
#
# Description: Enrichment analysis of human and archaic lineage-specific SNPs in circadian genes.
...
# OUTPUT DATA 
OUTPUT_FILE = '../data/enrichment_circadian_fixed_snps_genic.txt'
...
# LOAD FIXED VARIANTS
y_a, y_a_len = get_y_axis(AHMC)
y_h, y_h_len = get_y_axis(HHMC)

# FIND Q2: Fixed circadian
# Archaic
y_a_circadian = pd.merge(circadian_snps,y_a, on=['Chr','Start','End'])
y_a_circadian_len = len(y_a_circadian[['Chr','Start','End']])
# Human
y_h_circadian = pd.merge(circadian_snps,y_h, on=['Chr','Start','End'])
y_h_circadian_len = len(y_h_circadian[['Chr','Start','End']])
